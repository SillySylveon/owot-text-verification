...
(async function() {
    ...
    function checkVersion() {
        ...
        if (type !== '') {
            unsafeWindow.w.doAnnounce(`New ${type} found: OWOT Text Signatures v${lV}. <a href="${base_url}main.user.js">Click here to update.</a>`, 'textSignaturesUpdateBanner');
        }
    }
...
<br>
<button id="generate">Generate new key</button> or <a id="generate_password" style="color: blue; text-decoration: underline; cursor: pointer;">generate from a password</a>
<br><br><hr><br>
<div class="submitarea"></div>`;
...
<br><a style="font-size: 12px; color: #555">* Do not use your Uvias password.</a>`;
    ...
    pass.onSubmit(async function() {
        ...
        var usr = new Uint8Array(await s.digest("SHA-256", textEncoder.encode(unsafeWindow.state.userModel.username)));
        var pwd = textEncoder.encode(newDiv.querySelector('#user_password').value);
        ...
        var generated_private_key = fromHex("302e020100300506032b657004220420" + hash.hashHex);
        var gen_priv = await s.importKey("pkcs8", generated_private_key, "Ed25519", true, ['sign']);
        var gen_spki = await s.exportKey("jwk", gen_priv);
        delete gen_spki.d;
        gen_spki.key_ops = ['verify'];
        var gen_publ = await s.importKey("jwk", gen_spki, "Ed25519", true, ['verify']);
        var generated_public_key = await s.exportKey("raw", gen_publ);
